...
import tarfile
import traceback
from progressbar import ProgressBar, FileTransferSpeed, Bar, Percentage, ETA, RotatingMarker
...
class Stkintraday_Mode(object):
    ...
    def __init__(self, server_name):
        ...
        self.suspend_stock_list = []
        self.intraday_parameter_list = []
        ...
        self.parameter_dict_file_path = PARAMETER_DICT_FILE_PATH

        self.cfg_folder = self.tradeplat_file_folder + '/cfg'
        self.multifactor_folder = self.cfg_folder + '/intraday_multifactor/HighFreqCalculator'
        self.leadlag_folder = self.cfg_folder + '/intraday_leadlag/HighFreqCalculator'
        for temp_folder in (self.multifactor_folder, self.leadlag_folder):
    ...
    def __read_parameter_file(self, file_path, file_type):
        ...
        if not os.path.exists(file_path):
            self.email_content_list.append(
                '<font color=red>[Error]Parameter File:%s is missing!</font><br>' % file_path)
            return
        ...
        with open(file_path, 'rb') as fr:
            for line in islice(fr, 1, None):
                ...
                strategy_intraday_parameter.ticker = line_items[1].zfill(6)
        ...
        if file_type == 'StkIntraDayStrategy':
            self.intraday_parameter_list.extend(parameter_list)
    ...
    def __download_multifactor_files(self):
        multifactor_ticker_list = []
        for algo_parameter in self.intraday_parameter_list + self.basket_parameter_list:
            if algo_parameter.ticker in self.suspend_stock_list:
                self.email_content_list.append(
                    '<font color=red>[Error]ticker:%s is suspend!</font><br>' % algo_parameter.ticker)
                continue
            multifactor_ticker_list.append(algo_parameter.ticker)

        vwap_ticker_list = [parameter_item.ticker for parameter_item in self.basket_parameter_list if
                            parameter_item.ticker not in self.suspend_stock_list]
        intraday_ticker_list = [parameter_item.ticker for parameter_item in self.intraday_parameter_list if
                                parameter_item.ticker not in self.suspend_stock_list]
        ticker_date_dict = factordata_file_rebuild_job(vwap_ticker_list, intraday_ticker_list, self.multifactor_folder)

        table_list = []
        temp_parameter_list = []
        for algo_parameter in self.intraday_parameter_list:
            if algo_parameter.ticker in self.suspend_stock_list:
                continue

            dict_key = '%s|%s' % (algo_parameter.ticker, 'intraday')
            ticker_date_str = ticker_date_dict[dict_key]
            if ticker_date_str != '':
                temp_parameter_list.append(algo_parameter)
            else:
                ticker_date_str = 'Miss(Error)'
            table_list.append(['Intraday', algo_parameter.ticker, algo_parameter.parameter, ticker_date_str])
        self.intraday_parameter_list = temp_parameter_list
        ...
        for algo_parameter in self.basket_parameter_list:
            ...
            dict_key = '%s|%s' % (algo_parameter.ticker, 'vwap')
            ticker_date_str = ticker_date_dict[dict_key]
            if ticker_date_dict[dict_key] != '':
                temp_parameter_list.append(algo_parameter)
            else:
                ticker_date_str = 'Miss(Error)'
            table_list.append(['Vwap', algo_parameter.ticker, algo_parameter.parameter, ticker_date_str])
        self.basket_parameter_list = temp_parameter_list
        return table_list

    def __download_parameter_file(self):
        self.suspend_stock_list = stock_utils.get_suspend_stock()

        table_list = []
        table_list.extend(self.__download_multifactor_files())
        table_list.extend(self.__download_leadlag_files())
        table_list.sort()
    ...
    # 下载参数文件
    def __download_leadlag_files(self):
        temp_parameter_list = []
        ...
        self.leadLag_parameter_list = temp_parameter_list
        return []
    ...
    def __divide_tickers(self):
        ...
        all_parameter_list = self.intraday_parameter_list + self.leadLag_parameter_list + self.basket_parameter_list
    ...
    def __modify_strategyloader(self):
        ...
        conten_list = self.__build_strategyloader(self.intraday_parameter_list, 'StkIntraDayStrategy')
    ...
    def __modify_rb_sz_huabao(self):
        sz_ticker_dict = dict()
        for algo_parameter in self.intraday_parameter_list + self.leadLag_parameter_list:
        ...
        for rb_file_name in ('rb7_instruments.csv', 'rb8_instruments.csv', 'rb9_instruments.csv'):
            ...
            if end_index > len(message_content_list):
                end_index = len(message_content_list)
            save_message_list = message_content_list[start_index:end_index]
    ...
    def __modify_database(self):
        self.__save_algo_parameter(self.intraday_parameter_list, 'StkIntraDayStrategy')
    ...
    def __save_strategy_intraday_parameter(self):
        ...
        for parameter_info in self.intraday_parameter_list + self.leadLag_parameter_list:
    ...
    def __send_email(self):
        email_utils7.send_email_group_all('StkIntraDay File Build Over_%s' % self.server_name,
                                          ''.join(self.email_content_list), 'html')
