...
base_save_folder_template = 'Z:/dailyjob/Report/StockSelection_%s'
# base_save_folder_template = 'E:/dailyFiles/report/StockSelection_%s'
...
def __build_account_money_dict(server_model):
    query_sql = "select date,id,money from pf_account_money"
    ...
    for account_id, data in group:
        ...
        for ind in data.index.values:
            account_money_dict[data.at[ind, 'dict_key']] = data.at[ind, 'money']
    conn.close()
...
def pf_account_report(server_model, pf_account_db):
    ...
    for i in range(0, len(trading_day_list)):
        ...
        dict_query_key = '%s|%s' % (date_str, pf_account_db.id)
        ...
        if dict_query_key in trade_dict:
            ...
            for trade_db in trade_list:
                ...
                if trade_db.trade_type == 0:
                    ...
                    total_buy_money += float(trade_db.price) * abs(trade_db.qty) * float(instrument_db.fut_val_pt) * (margin_ratio + 0.000026)
...
* float(instrument_db.fut_val_pt) * (1 - 0.000026)
        ...
        for pf_position_db in pf_position_list:
            ...
            dict_query_key = '%s|%s' % (date_str, instrument_db.ticker)
            ...
            if instrument_db.type_id == 4:
                ...
                if abs(pf_position_db.short) <= abs(future_open_trade.qty):
                    future_equity += float(abs(pf_position_db.short)) * float(future_open_trade.price) * float(instrument_db.fut_val_pt) * margin_ratio \
+ (float(future_open_trade.price) - float(ticker_close_price)) * float(abs(pf_position_db.short)) * float(instrument_db.fut_val_pt)
                else:
                    for future_open_trade in future_open_dict[instrument_db.ticker]:
                        future_equity += float(abs(future_open_trade.qty)) * float(future_open_trade.price) * float(instrument_db.fut_val_pt) * margin_ratio \
+ (float(future_open_trade.price) - float(ticker_close_price)) * float(abs(future_open_trade.qty)) * float(instrument_db.fut_val_pt)
        ...
        account_money_key = '%s|%s' % (date_str, pf_account_db.id)
...
def __read_stock_daily_file(date_list):
    ...
    query_sql = "select t.TRADE_DT, t.S_INFO_WINDCODE, t.S_DQ_CLOSE from dump_wind.ASHAREEODPRICES t where \
t.TRADE_DT in (%s)" % ','.join(query_date_list)
...
def __read_future_daily_file(date_list):
    ...
    server_model = server_constant.get_server_model('local118')
    session_portfolio = server_model.get_db_session('dump_wind')
    query_sql = "select TRADE_DT,S_INFO_WINDCODE,S_DQ_CLOSE from dump_wind.CINDEXFUTURESEODPRICES where \
TRADE_DT in (%s)" % ','.join(query_date_list)
...
def __email_algo_pf_position(pf_account_db_list):
    ...
    for strategy_base_name in strategy_name_list:
        ...
        df.to_csv(file_path, header=False)
        attachment_file_list.append(file_path)
    email_utils.send_email_path('多因子策略收益统计', '', ','.join(attachment_file_list), 'html')
...
if __name__ == '__main__':
    daily_return_report_job()

    # server_name = 'huabao'
    # now_date_str = '2017-02-14'
